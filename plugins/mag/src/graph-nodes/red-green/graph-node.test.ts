...
import { DeadTestAtBirth, HarnessError, PathsTouched, StillRed, TestDisputed } from "mag/graph-nodes/red-green/errors"
 ...
 * implement's leftovers are committed by their node). The per-path test command is answered from
 * `colours`, one entry per `assert-red` call in order, so a test scripts the whole loop by listing
 * what each classification should see. `touched` is what `paths-untouched`'s diff reports.
 */
const loopShell = (options: { colours: readonly number[]; touched?: string }) => {
  const calls: string[][] = []
...
describe("red-green", () => {
  ...
  test("the straight line: red at the red sha, green after implement, one pass each, spend folded", () =>
  ...
  test("a harness error routes back to write-red with its own wording, distinct from a dead test", () =>
    withRunRoot(async (runRoot) => {
      const agent = loopAgent()
      const result = await runLoop(INPUT, loopShell({ colours: [2, 1, 0] }).service, agent.service, runRoot)

      expect(Result.isSuccess(result)).toBe(true)
      const writes = agent.requests.filter(isWritePrompt)
      expect(writes[1]!.prompt).toContain(`Never ran at bbb222: ${TEST}`)
      expect(writes[1]!.prompt).not.toContain("Already green")
    }))
  ...
  test("the write cap spent refails the last verdict itself: DeadTestAtBirth, HarnessError", () =>
    withRunRoot(async (runRoot) => {
      ...
      const broken = await runLoop({ ...INPUT, cap: 0 }, loopShell({ colours: [2] }).service, loopAgent().service, runRoot)
      expect(Result.isFailure(broken)).toBe(true)
      if (Result.isFailure(broken)) expect(broken.failure).toStrictEqual(new HarnessError({ broken: [TEST], sha: "bbb222" }))
    }))
  ...
  test("the implement cap spent refails StillRed with the last head's evidence", () =>
    withRunRoot(async (runRoot) => {
      ...
      expect(result.failure).toStrictEqual(new StillRed({ red: [TEST], broken: [], sha: "ddd444" }))
